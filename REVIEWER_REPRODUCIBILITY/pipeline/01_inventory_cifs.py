...
def inventory(source,limit=None,pdb_id=None,output_dir=None):
 ...
 for path in c.source_paths(source,limit,pdb_id):
  ...
  rows.append(base)
 mdir=Path(output_dir) if output_dir else c.ROOT/'outputs'/'inventory'; mdir.mkdir(parents=True,exist_ok=True); out=mdir/'FROZEN_CIF_CORPUS_MANIFEST.csv'
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--source',default=str(c.DEFAULT_SOURCE));p.add_argument('--limit',type=int);p.add_argument('--pdb-id');p.add_argument('--output-dir');a=p.parse_args();r=inventory(Path(a.source),a.limit,a.pdb_id,a.output_dir);print(f'inventoried {len(r)} CIF files')
