...
def ensure_synonyms(db):
    ...
    # Synonym retrieval is deployment-only.  It is intentionally excluded from
    # the scientific Stage-14 projection and is available under web_compat/.
    sources = ["scientific pipeline: no external synonym source"]
...
def run(database: str):
    ...
    with c.dbconn(database) as db:
        ...
        synonym_info = ensure_synonyms(db)
