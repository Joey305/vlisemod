...
def main() -> None:
    ...
    c_contact_chains = resolve_column(
        df, ["contacting_protein_chains", "target_chains", "stage09_target_chains"], required=False
    )
    ...
    for idx, row in df.iterrows():
        ...
        if category in NO_CONTACT_CATEGORIES:
            reconciled = ""
            status = "UNRESOLVED_NO_CONTACT_EVIDENCE"
        elif not descriptions:
            # The prior audit category is authoritative here: a row can have
            # Stage-09 contact context but lack usable mmCIF entity metadata.
            # Do not mislabel that as absence of direct-contact evidence.
            reconciled = ""
            status = "MANUAL_REVIEW_CONTACT_WITHOUT_ENTITY_METADATA"
            ...
            template_rows.append(
                {
                    ...
                    "distinct_pdbs": r["distinct_pdbs"],
                    "current_targets": r["current_targets"],
                    "folder_labels": r["folder_labels"],
                    "pdb_examples": r["pdb_examples"],
    ...
    tdf = pd.DataFrame(
        template_rows,
        columns=[
            "virus_name", "entity_description", "canonical_target",
            "occurrence_count", "distinct_pdbs", "current_targets",
            "folder_labels", "pdb_examples", "note"
        ],
    )
    if not tdf.empty:
        tdf = tdf.sort_values(
            ["occurrence_count", "distinct_pdbs", "virus_name", "entity_description"],
            ascending=[False, False, True, True], kind="mergesort"
        )
    tdf.to_csv(args.outdir / "taxonomy_manual_overrides_template.csv", index=False)
    ...
    summary = {
        ...
        "contact_without_entity_metadata_occurrences": int(
            (rdf["taxonomy_reconciliation_status"] == "MANUAL_REVIEW_CONTACT_WITHOUT_ENTITY_METADATA").sum()
        ),
        "auto_resolved_prior_disagreement_occurrences": int(
            ((rdf["taxonomy_reconciliation_status"] == "AUTO_RESOLVED_EXACT") &
             (rdf[c_category] == "CONTACT_ENTITY_CLASSIFICATION_DISAGREEMENT")).sum()
        ),
