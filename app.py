...
# --- AUTH SETUP (safe loader + bad-cookie guard + safe next) ---
...
from flask import (
    request, redirect, url_for, render_template, render_template_string,
    session, flash
)
...
app.config.update(
    ...
    SESSION_REFRESH_EACH_REQUEST=True,   # sliding expiration while active
    # If you serve over HTTPS only, consider uncommenting:
...
def _ensure_users_table():
    with sqlite3.connect(USERS_DB) as con:
        ...
        """)
        # Ensure columns exist (safe no-ops if already there)
        ...
        # Ensure unique index on email (in case older DB missed it)
        try:
...
def create_user_if_missing(email: str, password: str):
    """Safe to call repeatedly; it only inserts if not present."""
    _ensure_users_table()
    with sqlite3.connect(USERS_DB) as con:
        row = con.execute("SELECT 1 FROM users WHERE email=?", (email.strip().lower(),)).fetchone()
        if row:
            return False
        con.execute(
...
class User(UserMixin):
    def __init__(self, uid: int, email: str, password_hash: str):
        self.id = uid
        self.email = email
...
def load_user(user_id: str):
    ...
    if not row:
        return None
    uid, email, password_hash = row
    u = User(uid, email, password_hash)
    return u


# 2) Scrub bad cookies before any request
@app.before_request
def _clear_bad_login_cookie():
    uid = session.get("_user_id")
    if uid in (None, "", "None", "null", "undefined"):
        session.pop("_user_id", None)

# ---- Helpers ----
def _is_safe_url(target: str) -> bool:
    """
    Only allow redirects to same host; otherwise fallback to index.
    This avoids cookie-domain mismatches (e.g., posting to localhost then
    redirecting to a tailscale hostname).
    """
    if not target:
        return False
...
# ---- Bad-cookie cleaner: run before anything touches current_user ----
@app.before_request
def _clear_bad_login_cookie():
    # Flask-Login stores the user id at session key "_user_id"
    uid = session.get("_user_id")
    if uid in (None, "", "None", "null", "undefined"):
        # remove the bad key so Flask-Login treats this as anonymous
        session.pop("_user_id", None)

# # ---- Global gate: everything requires login except a small allowlist ----
# EXEMPT_PATH_PREFIXES = (
#     "/login", "/logout", "/force-logout",
#     "/static", "/favicon.ico", "/healthz", "/__debugger__" , "/copy"
# )
EXEMPT_PATH_PREFIXES = (
    "/login", "/logout", "/force-logout",
    "/static", "/favicon.ico", "/healthz", "/__debugger__",
    "/copy",                    # legacy redirect to standalone PROTAC Builder
    "/run-drug-analysis",       # legacy compatibility response
    "/download",                 # 🔥 allow PDF download
    "/"
)
...
#     p = (request.path or "")
#     if p.startswith(EXEMPT_PATH_PREFIXES):
#         return
#     if current_user.is_authenticated:
#         # optional idle timeout (extra safety beside PERMANENT_SESSION_LIFETIME)
#         last = session.get("last_seen")
#         now = int(time.time())
#         if last and now - last > app.config["PERMANENT_SESSION_LIFETIME"].total_seconds():
#             # idle too long; log out and force re-login
#             sid = session.pop("sid", None)
#             try:
#                 if sid:
#                     _delete_session(sid)  # your existing concurrency helper
#             except Exception:
#                 pass
#             logout_user()
#             session.clear()
...
def require_login_everywhere():
    ...
    # Existing exempt paths (static, login, etc.)
    if p.startswith(EXEMPT_PATH_PREFIXES):
        return
...
# ---- Routes: login/logout & health; keep your existing _start_session/_delete_session ----
...
def login():
    ...
    # unified error so we never leak which exists
    if not row:
    ...
    # success
    login_user(User(uid, em, pw_hash))

    # clear any stale sid, then start a new tracked session (your concurrency guard)
    ...
    sid, err = _start_session(uid, request)   # ← your existing concurrency limiter
    if not sid:
        logout_user()
        return render_template("login.html", error=err), 429
...
# Quick “panic” route if you ever get stuck with a weird cookie
...
def enforce_idle_timeout():
    if not current_user.is_authenticated:
        return
    sid = session.get("sid")
    # If sid missing or revoked, force logout
    if not sid or not _session_exists(sid, int(current_user.id)):
        session.pop("sid", None)
        logout_user()
        flash("Your session was ended (another login or timeout). Please sign in again.", "warning")
        return redirect(url_for("login"))
    ...
    if now - last > timeout:
        # remove this sid from DB and log out
        _delete_session(sid)
        session.pop("sid", None)
        logout_user()
    ...
    # still active -> update heartbeat (optional: skip certain endpoints)
    EXCLUDE_FROM_REFRESH = set()  # e.g., {"/get_viruses"} to avoid background pings extending sessions
    if request.path not in EXCLUDE_FROM_REFRESH:
        session["last_seen"] = now
        session.permanent = True
        _touch_session(sid)
...
# how many concurrent sessions a single user may have
MAX_SESSIONS_PER_USER = int(os.environ.get("MAX_SESSIONS_PER_USER", 2))

# behavior when user hits the cap: "DENY" or "EVICT" (kick out oldest)
SESSION_CONCURRENCY_MODE = os.environ.get("SESSION_CONCURRENCY_MODE", "DENY").upper()
...
def _prune_expired_sessions(con, user_id=None):
    cutoff = _now() - _cutoff_seconds()
    if user_id is None:
        con.execute("DELETE FROM sessions WHERE last_seen < ?", (cutoff,))
    else:
        con.execute("DELETE FROM sessions WHERE user_id = ? AND last_seen < ?", (user_id, cutoff))

def _count_active_sessions(con, user_id):
    cutoff = _now() - _cutoff_seconds()
    row = con.execute("SELECT COUNT(*) FROM sessions WHERE user_id = ? AND last_seen >= ?",
                      (user_id, cutoff)).fetchone()
    return row[0] if row else 0

def _oldest_active_session(con, user_id):
    cutoff = _now() - _cutoff_seconds()
    row = con.execute("""SELECT sid FROM sessions
                         WHERE user_id = ? AND last_seen >= ?
                         ORDER BY last_seen ASC LIMIT 1""", (user_id, cutoff)).fetchone()
    return row[0] if row else None

def _start_session(user_id, request):
    """Create a new session record (respect MAX_SESSIONS_PER_USER).
       Returns (sid, error_message_or_None)."""
    _ensure_sessions_table()
    with sqlite3.connect(USERS_DB) as con:
        _prune_expired_sessions(con, user_id)
        active = _count_active_sessions(con, user_id)
        if active >= MAX_SESSIONS_PER_USER:
            if SESSION_CONCURRENCY_MODE == "EVICT":
                oldest = _oldest_active_session(con, user_id)
                if oldest:
                    con.execute("DELETE FROM sessions WHERE sid = ?", (oldest,))
            else:
                return None, f"Too many active sessions (max {MAX_SESSIONS_PER_USER}). Please log out on another device."
        sid = secrets.token_urlsafe(32)
        ua = (request.headers.get("User-Agent") or "")[:256]
        ip = (request.headers.get("X-Forwarded-For") or request.remote_addr or "")[:64]
        now = _now()
        con.execute("""INSERT INTO sessions(sid, user_id, created_at, last_seen, user_agent, ip)
                       VALUES(?,?,?,?,?,?)""", (sid, user_id, now, now, ua, ip))
        con.commit()
        return sid, None

def _session_exists(sid, user_id):
    cutoff = _now() - _cutoff_seconds()
    with sqlite3.connect(USERS_DB) as con:
        row = con.execute("""SELECT 1 FROM sessions
                             WHERE sid = ? AND user_id = ? AND last_seen >= ?""",
                          (sid, user_id, cutoff)).fetchone()
        return bool(row)

def _touch_session(sid):
    with sqlite3.connect(USERS_DB) as con:
        con.execute("UPDATE sessions SET last_seen = ? WHERE sid = ?", (_now(), sid))
        con.commit()
