...
def merge_files(base_input_dir, virus_names, output_file):
    with open(output_file, 'w', newline='') as outfile:
        writer = csv.writer(outfile, delimiter='\t')
        # Write the header to the output file
        header = ['virus_name', 'pdb_file', 'ligand', 'SMILES', 'Functional_Groups']
        writer.writerow(header)

        for virus_name in virus_names:
            input_file = Path(base_input_dir) / 'Sorted_PDB_Files' / virus_name / 'with_ligands_smiles_functional_groups_GROUPED.txt'
...
def main():
    ...
    output_file = 'merged_grouped_functional_groups.txt'
    ...
    # Merge files
    merge_files(base_input_dir, virus_names, output_file)
