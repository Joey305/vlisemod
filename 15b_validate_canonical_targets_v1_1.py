...
Version 1.1 fixes the Stage-12 guardrail so the frozen 9,462-row denominator is
checked only for the current PROTACability evidence generation
(protacability-cif-v2.8), matching the existing Stage-15 release validator.
Historical protacability_assessment rows are intentionally retained and are not
a failure.
"""
...
VERSION = "canonical-target-browser-validation-pass9-v1.1"
...
EXPECTED_PROTACABILITY_ASSESSED_INSTANCES = 6784
...
def main():
    ...
    try:
        ...
        if exists(db, "table", "protacability_assessment"):
            ...
            metrics["protacability_assessment_historical_rows"] = historical_rows
            ...
            check(
                f"Stage-12 assessment rows ({PROTACABILITY_VERSION}) unchanged",
                current_rows,
                EXPECTED_PROTACABILITY_ASSESSMENT_ROWS,
            )

            current_instances = db.execute(
            ...
            check(
                f"Stage-12 assessed ligand instances ({PROTACABILITY_VERSION}) unchanged",
                current_instances,
                EXPECTED_PROTACABILITY_ASSESSED_INSTANCES,
            )

            bad_basis = db.execute(
                """SELECT count(*)
                   FROM protacability_assessment
                   WHERE method_version=?
                     AND (
            ...
            check(
                f"Stage-12 direct-contact basis ({PROTACABILITY_VERSION}) unchanged",
                bad_basis,
                0,
            )
