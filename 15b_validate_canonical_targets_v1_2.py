...
VALIDATOR_VERSION = "canonical-target-browser-validation-pass9-v1.2"
...
def validate(db: sqlite3.Connection) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    eligibility_col = next(
        (
            ...
            if c in authority_cols
        ),
        None,
    )
    if eligibility_col is None:
        add_check(checks, "authority eligibility partition", EXPECTED_ELIGIBILITY, "MISSING_COLUMN")
    else:
        eligibility = grouped_counts(
            db,
            f"""
            SELECT {eligibility_col}, count(*)
            FROM canonical_ligand_targets
            GROUP BY {eligibility_col}
            ORDER BY {eligibility_col}
            """,
        )
        add_check(checks, "authority eligibility partition", EXPECTED_ELIGIBILITY, eligibility)
    ...
    if canonical_id_col is None:
        ...
        invalid_ids = int(
            scalar(
                ...
                SELECT count(*)
                FROM canonical_ligand_targets
                WHERE {canonical_id_col} IS NULL
                   OR trim({canonical_id_col})=''
                   OR {canonical_id_col} GLOB '*[^a-z0-9_]*'
                """,
            )
            or 0
        )
